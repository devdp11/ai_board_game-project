...
class MinimaxMijnlieffPlayer(MijnlieffPlayer):
    ...
    def __heuristic(self, state: MijnlieffState):
        grid = state.get_grid()
        longest = 0
    ...
    def minimax(self, state: MijnlieffState, depth: int, alpha: int = -math.inf, beta: int = math.inf,
                is_initial_node: bool = True):
        # first we check if we are in a terminal node (victory, draw or loose)
        if state.is_finished():
            return {
                MijnlieffResult.WIN: 40,
                MijnlieffResult.LOOSE: -40,
                MijnlieffResult.DRAW: 0
